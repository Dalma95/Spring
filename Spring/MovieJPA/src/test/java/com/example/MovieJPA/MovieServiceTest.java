...
public class MovieServiceTest {
    ...
    void setUp(){
        ...
        movieDetails.setLanguage("english");
        ...
        testDto.setActorNames(Set.of("Leonardo Di Caprio"));
        testDto.setDuration(148);
        testDto.setLanguage("english");
        testDto.setSynopsis("bla bla");

    }

    @Test
    void save_ShouldRetunrSavedMovie(){
    ...
    @Test
    void delete_ShoulDeleteMovieById(){
    ...
    @Test
    void findByRatingGreaterThan_ShouldReturnMovies(){
    ...
    @Test
    void addMovieToWatchList_ShouldUpdateMovieShouldSendMail() throws EmailSendingException {
        when(movieRepo.findById(1L)).thenReturn(Optional.of(testMovie));
        when(movieRepo.save(testMovie)).thenReturn(testMovie);
        doNothing().when(emailService).sentEmail(anyString(),anyString(),anyString());

        MovieDto updatedMovie = movieService.addMovieToWatchedList(1L, "test@gmail.com");
//        assertTrue(updatedMovie.isWatchlist());
    ...
    @Test
    void addMovieToWatchList_ShouldThrowException_WhenEmailFails() throws EmailSendingException {
        when(movieRepo.findById(1L)).thenReturn(Optional.of(testMovie));
        when(movieRepo.save(testMovie)).thenReturn(testMovie);
        doThrow(new EmailSendingException("Faild to send email.")).when(emailService).sentEmail(anyString(),anyString(),anyString());

        ServiceException exception = assertThrows(ServiceException.class, () ->
                movieService.addMovieToWatchedList(1L,"test@gmail.com"));
        assertEquals("Movie updated but email sending faild", exception.getMessage());
